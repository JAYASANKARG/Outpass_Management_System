...
require('../database/database.php');
	session_start();
	if(!isset($_SESSION['watchman']))
	{
		header('Location: ..');
  }
  
  $query = $db->prepare('SELECT * FROM outpass where status="closed"');
  $query->execute();
  $apply=$query->rowcount();

  $query1=$db->prepare('SELECT * FROM outpass where status="warden-ok"');
  $query1->execute();
  $pending=$query1->rowcount();

  $query2=$db->prepare('SELECT * FROM outpass where status="watchman-ok"');
  $query2->execute();
  $approval=$query2->rowcount();

  $query3=$db->prepare('SELECT * FROM outpass where watch_man="no"');
  $query3->execute();
      ...
      <div class="row">
        <div class="container-fluid">
          <div class="pull-left">
            ...
            <span><?php $user=$_SESSION['watchman'];
            echo $user ?><span>
            </div>
          </div>
              ...
              <div class="panel panel-yellow">
                ...
                <div class="table-responsive">
                  <table class="table table-striped" id="example">
                    ...
                    <tr>
                    <td>
				            <form action="../database/outpass_auth_watch_man.php" method="POST">
					          <input type="hidden" value="<?php echo $one["outpass_number"] ?>" name="number" >
              ...
              <div class="panel panel-green">
                ...
                <div class="table-responsive">
                  <table class="table table-striped" id="example">
                    ...
                    <tr>
                    <td>
				            <form action="../database/outpass_close.php" method="POST">
					          <input type="hidden" value="<?php echo $one["outpass_number"] ?>" name="number" >
                    <button type="submit" class="btn btn-success" name="yes">Approval</button>
                    </td>
                    <td>
